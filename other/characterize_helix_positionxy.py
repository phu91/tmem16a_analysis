...
# FUNCTIONS
def alignment_ref(ref_struct,frame):
    ref = mda.Universe(top_file, top_file)
    align.alignto(frame, ref, select=ref_struct, weights="mass")
...
with open("HELIXPOSXY_profile_%s.dat"%(systemname),"w+") as rdf_out:
    rdf_out.write("#helix chain x y\n")
    # Average Structure
    for ts in u.trajectory[::traj_skip]:
        for ind, (helixa,helixb,helixa_str,helixb_str) in enumerate(zip(helixa_list,helixb_list,helixa_str_list,helixb_str_list)):
            alignment_ref(helixa_str,u.trajectory[ts])
