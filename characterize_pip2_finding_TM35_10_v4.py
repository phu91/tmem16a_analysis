...
pip2_list_found =[]
for ts in tqdm(u.trajectory[::traj_skip*10],desc='Finding PIP2'):
...
df_pip2 = pd.DataFrame(pip2_list_found,columns=['resid','position']).drop_duplicates().reset_index()
if len(df_pip2)!=0:
    print()
    print(df_pip2)
else:
    ...
    if len(df_pip2)!=0:
        for ts in tqdm(u.trajectory[::traj_skip],desc='Tracking PIP2'):
            for res in df_pip2.resid:
                pip2_567_A = u.select_atoms("(around 10 (segid PROA and resid 567 and name NZ)) and (resname PLPI24 and name P4 and resid %s)"%(res),updating=True)
                pip2_567_B = u.select_atoms("(around 10 (segid PROB and resid 567 and name NZ)) and (resname PLPI24 and name P4 and resid %s)"%(res),updating=True)
                pip2_887_A = u.select_atoms("(around 10 (segid PROA and resid 887 and name NZ)) and (resname PLPI24 and name P4 and resid %s)"%(res),updating=True)
                pip2_887_B = u.select_atoms("(around 10 (segid PROB and resid 887 and name NZ)) and (resname PLPI24 and name P4 and resid %s)"%(res),updating=True)
                # print(ts.frame,pip2_567_A,pip2_567_B,pip2_887_A,pip2_887_B)
                if len(pip2_567_A)!=0:
                    at567A=1
                else:
                    at567A=0
                ...
                if len(pip2_887_B)!=0:
                    ...
                    at887B=0
            ofile.write("FRAME %s 567A %s 567B %s 887A %s 887B %s\n"%(ts.frame,at567A,at567B,at887A,at887B))
            ofile.flush()
            # print(ts.frame,at567A,at567B,at887A,at887B)
                # print(pip2_A.resids)
                # print(pip2_B.resids)
