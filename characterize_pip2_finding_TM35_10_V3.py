...
important_pip2_A_1 = u.select_atoms("(around %s (segid PROA and resid 567 and name NZ)) and (resname PLPI24 and name P*)"%(cutoff),updating=True)
important_pip2_A_2 = u.select_atoms("(around %s (segid PROB and resid 887 and name NZ)) and (resname PLPI24 and name P*)"%(cutoff),updating=True)
important_pip2_B_1 = u.select_atoms("(around %s (segid PROB and resid 567 and name NZ)) and (resname PLPI24 and name P*)"%(cutoff),updating=True)
important_pip2_B_2 = u.select_atoms("(around %s (segid PROA and resid 887 and name NZ)) and (resname PLPI24 and name P*)"%(cutoff),updating=True)
...
with open("PIP2_TM345_10_PROFILE_%s_V2.dat"%(systemname),'w+') as ofile:
    ...
    for ts in tqdm(u.trajectory[::traj_skip*1],desc='Finding PIP2'):        ## CHANGE SKIP TO MAKE THE FIND FASTER
        ...
        print("chain 567B: %s"%(important_pip2_B_1.resids))
        print("chain 887A: %s"%(important_pip2_B_2.resids))
        print("chain 567A: %s"%(important_pip2_A_1.resids))
        print("chain 887B: %s"%(important_pip2_A_2.resids))
        chainA_887=0
        chainB_887=0
        if len(important_pip2_B_2)!=0 and len(important_pip2_B_1)!=0:
            # if (np.unique(important_pip2_B_2.resids)).any()==(np.unique(important_pip2_B_1.resids)).any():
            chainA_887=1
            print("===>FOUND 887A")
        else:
            chainA_887=0
