...
for ts in tqdm(u.trajectory[::traj_skip]):
    ...
    dist_564_641_A = distance_by_chain(ts.frame,K584_A,K641_A,"A")
    dist_564_641_B = distance_by_chain(ts.frame,K584_B,K641_B,"B")

    df = pd.concat([df,dist_567_A_887_B,dist_567_B_887_A,dist_564_641_A,dist_564_641_B])
    del dist_567_A_887_B,dist_567_B_887_A,dist_564_641_A,dist_564_641_B

# print(df)
with open("TM_34_56_PROFILE_%s.dat"%(systemname),'w+') as ofile:
    ofile.write("# DISTANCE BETWEEN K567(A_LOOP45) to K887(B_THIRD_CA), K584 to K641 (same chain)\n")
    ofile.write("#frame	resid	chain	distance\n")

df.to_csv("TM_34_56_PROFILE_%s.dat"%(systemname),sep='\t',index=False,mode='a',header=False)
print("\n==> DATA saved to TM_34_56_PROFILE_%s.dat" %(systemname))
